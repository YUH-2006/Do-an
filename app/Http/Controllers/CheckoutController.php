...
class CheckoutController extends Controller
{
    ...
    private function getCartItems(Request $request): array
    {
        ...
        if ($cart) {
            return $cart->items()->with('product')->get()->map(fn(CartItem $r) => [
                'id' => (int) $r->product->id,
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('checkout.success', $order->id)->with('success', 'Đơn hàng của bạn đã được đặt thành công!');
    }
